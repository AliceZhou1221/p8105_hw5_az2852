...
Problem 2 When designing an experiment or analysis, a common question is
...
Power increases as effect size increases. The increase in power is sharp
at first and then gradually slows down as power gets close to 1.
...
![](p8105_hw5_az2852_files/figure-gfm/unnamed-chunk-8-1.png)<!-- --> The
average estimate mu_hat for which the null is rejected is not
...
variables include reported data, victim’s first and last name,
victim_race <chr>, victim_age <chr> victim_sex <chr>, city, state,
latitude and longitude, and disposition <chr>. There is a significant
amount of missing data in variables victim_race, victim_age, and
victim_sex.
...
ggplot(results_df, aes(x = reorder(city_state, estimate), y = estimate)) +
  geom_point(color = "blue", size = 2) +  # Plot the estimated proportion as points
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +  # Add error bars (confidence intervals)
  labs(
    x = "City",
  ...
  coord_flip()  # Flip the coordinates for better visibility of city names
```
